...
import os
import json
from CloudMusic.Music163api import Music163API
import difflib
...
class DownSong(Music163API):
    ...
    def single_song(self, name, singer, folder_name, flag):
        try:
            singer_box = singer.replace('Presents ', '').replace('Featuring ', '')
            song_list = self.search(name + ' ' + singer_box)
        except Exception as e:
            print("输入格式有误，请从新操作！, {}".format(e))

        if not song_list:
            print("there is no match result from CloudMusic")
            return 10, None
        ...
        for i in range(0, lens):
            song_id, song_name, song_singer = song_list[i]['id'], song_list[i]['name'], song_list[i]['ar'][0]['name']

            diff_name = difflib.SequenceMatcher(None, name.upper(), song_name.upper()).quick_ratio()
            diff_singer = difflib.SequenceMatcher(None, singer.upper(), song_singer.upper()).quick_ratio()

            if song_singer in singer or diff_singer > 0.1:
                if name == song_name or diff_name > 0.9:
                    song_url, song_size = self.get_music_url(song_id)
                    if song_url:
                        return self.down_single_music(song_url, song_size, name, singer, i, folder_name, flag), song_id
                    else:
                        print('{}_{} 没有版权无法下载~~'.format(song_name, song_singer))
                        return 5, song_id
                if ('Remix' in song_name) and diff_name > 0.4:
                    song_url, song_size = self.get_music_url(song_id)
                    if song_url:
                        if self.down_single_music(song_url, song_size, name, singer, i, folder_name, flag) == 1:
                            return 2, song_id
                        return 3, song_id
                    else:
                        print('{}_{} 没有版权无法下载~~'.format(song_name, song_singer))
        ...
        print("未找到匹配条件{} {}的歌曲".format(name, singer))
        return 4, song_id
...
def scheduled_downloader(name, singer, folder_name, flag):
    menu = Menu()
    return menu.down_song.single_song(name, singer, folder_name, flag)
