...
from CloudMusic.encrypted import Encrypted
...
class Music163API(object):
    ...
    def request_post(self, url, params):
        data = self.encrypt.encrypted_request(params)
    ...
    def down_music(self, url, size, name):
        ...
        try:
            ...
            print("{}下载出错 Exception:{}".format(name, e))

    def down_single_music(self, url, size, name, singer, i, folder_name, flag):
        """
        save music
        """
        if flag is False:
            return 1
        path = './Music/{}'.format(folder_name)
        if not os.path.exists(path):
            os.mkdir(path)
        response = requests.get(url)

        file_path = os.path.join(path, re.sub(self.rstr, "", name) + '_' + re.sub(self.rstr, "", singer) + '.mp3')
        try:
            with open(file_path, 'wb') as f:
                f.write(response.content)
                print("the searched ''%dth''   [%s_%s] 下载完成 共 %.2f M" % (i, name, singer, size / 1024 ** 2))
                f.close()
                return 1
        except Exception as e:
            f.close()
            print("{}下载出错".format(name, e))
            return 3

    def down_single_music3(self, url, size, name, singer, i, folder_name):
        """
        save music
        """
        path = './Music/{}'.format(folder_name)
    ...
    def playlist_song(self, playlist_id, description=False):
        """
        Get a song list of playList pages
    ...
    def get_fans(self, user_id, current_page=1):
        ...
        params = {'offset': str((current_page - 1) * 20), 'limit': '20', 'total': 'true', 'csrf_token': '',
                  'userId': str(user_id)}
        result = self.request_post(url, params)
        return result
...
def main():
    ...
    while more:
        ...
        followed_list = result['followeds']
        more = result['more']
